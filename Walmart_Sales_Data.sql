...
SELECT 
    ...
    COUNT(DISTINCT day_name) AS Day_Names, 
    COUNT(DISTINCT month_name) AS Month_Names
FROM salesData;
...
# What are the sales figures for each city?
SELECT DISTINCT city, ROUND(SUM(TOTAL), 2) AS Total_Sales FROM salesData GROUP BY city ORDER BY Total_Sales DESC;
...
# To calculate the cost of goods sold (COGS), use the formula:
